...
とαと VDAC2 の連立となる。Gemini に任せたところ α=0.044, VDAC2 = 0.6067V となった。irb
で検算したが大丈夫。さらに R2 = 1kΩとすると、R1=21.727kΩなので R1=22kΩで良いだろう。

今度は逆に辿っていくことになる。α=0.04348 となる。検討しているマイコンは RL78/G24 で 
[データシート](https://www.renesas.com/ja/document/mah/rl78g24-users-manual-hardware-rev120?r=25442531)
21.1 節の式から、VDAC2 = VDD * DAC2/1024 となるので DAC2=124.2、整数なので 124 ということになる。

つまり VDAC = 5/1024*124= 0.6055V となる。

α=0.04348, VDAC2=0.6055V を式8, 式4 に入れて VDAC1 での VFB, Vout の振れ幅をもう一度確認する。
...
Vout と VDAC1 の関係式が欲しいので 式8 と式4から式 11
```math
VDAC1=8.313857 -4.043794 VOUT
```

検算すると VDAC1(VOUT=0.82) = -0.016V, VDAC1(VOUT=2.06) = 4.998V となった。まあ良いでしょ
う。


次は 5V 駆動の RL78/G24 で ADC で電池電圧を測ることを考える。あれこれ考えたがぱっとした考
えはなく、シャントレギュレータという基準電圧を作るためのものがあるというので、それにしよう。
秋月で 2.49V の TA76L431S 東芝、というのがあるから、それにしよう。なお Gemini によると 1%
の誤差というのは、1% でふらつくというよりも、出荷時の誤差と考えたら良いらしい。嘘か本当か
は確認できなかった。2550 カウントくらい使えるから、まあ十分でしょう。

次は電圧測定の制御方法を考える。先生の MAX712 は C/2 スピードであれば 84秒に一回電圧比較を
している。
最悪 84秒間、充電完了であることに気づかずにフル充電掛けるわけだが、NiMH は化学反応が終った
ら、あとは内部抵抗として熱になるだけなので、放熱とバランスさえ取れていれば多少のことは問題
ないらしい。
そして 2.5mV の電圧低下を観測したら、C/16 のトリクル充電となる。エネループ
1900mAh なら 119mA。

さて 2.49V に対する 12bit ADC の分解能は 0.6079V/カウント。となると 2.5mV は 4カウント。
RL78/G24 の 43.6.1 A/D 特性によると ADVREF=2.49V のときの誤差は 7.5+2.51*0.75=9.4カウント。
この誤差が 3σ、正規分布に載っていると仮定したら、σ=3.133。
n 回の平均を取ったときの標準偏差 σ_meanは、こうなる。

```math
\sigma_{mean} = \frac{\sigma}{\sqrt{n}}
```

n=64 で σ=1.175, n=128 で σ=0.831。まあどっちかでしょう。

ADC の値をこんな感じで n=64 か 128 で平滑化すれば良いだろう。この平滑化は単純移動平均 SMA
と言うらしい。

```
ADC_{n}=\frac{1}{n}ADC_{n} + \frac{n-1}{n}ADC_{n-1}
```

動作としてはこんな感じか。

1. 84sec/n ごとに ADC を掛けて、SMA な ADC を取得
2. 最大値の確認と、更新
3. n回 (84秒) 連続で最大値を下回れば、終了
   - MAX712 先生は 84秒過ぎてから電圧測定、比較だけど、SMA なら大丈夫でしょう。
   - ADC_1 = ADC_0 = 一番最初の電池電圧としておけばまあ、序盤の急激な上昇もなめらかな数値
     となるでしょう。

4. 132 分でも終了

こんな感じだろう。つまり 64回とか 128回も比較をすれば、電圧が上昇傾向にあるなら、誤差があるとは言え、
一度くらいは最大値に届くだろう。

それから、電圧の制御も補正を入れてみる。

1, Vout が VBAT の+0.47V となる DAC1 を設定で始める。
2, DAC1 の Vout に対する分解能 (2.06-0.82)/1024 から、Vout の過不足分をの補正量を求める
3, 2回目の DAC1 を求めるときに VBAT+0.47 となるカウント値に、#2 の補正量を加える

数式上は、線形だからバチっと決まることになってるが、実現象はたぶん非線形なので、下手したら
振動するかも。なので、補正量は 0.5 とか係数を掛けたほうが良いかもしれない。あと、ここでは
ADC の誤差は問題にならないので、ここで使うのは移動平均ではなく、そのときの値である。
0.47V/2.5V*4096=770カウントであり、4カウント程度刷れたところで問題ではない。

上手く行くなら抵抗 0.47Ωより小さくできそうだが、1A, 0.47Ωで 0.47W。そんなに気にするほど
でも無い。前作までの 1Ωよりも改善してんだから、とりあえずやってみよう。

------
良く考えたら、終了できない。LXDC55 の EN が使えない。PchMOSFET でも通して、強制終了か？
