...
で、諦めきれずにいろいろ考えて、LXDC55 の FB 入力が低いほど電圧が上がるから、電源3.3V から
電池電圧を引いて、ある程度倍率を掛けたものを FB に入力したらどうかと思ったんだが、なかなか
良い感じに合わせこむのが難しい。

でいろいろ考えて、マイコンの DAC と DAC で挟みこむことでフルレンジで使えるような気がしてき
た。
...
よって 式6
```math
V3=\frac{R2}{R1+R2} V1 + \frac{R1}{R1+R2} V2
```

さて今 V1 への入力として 0-3.3V のフルレンジを使いたい。
そして
[VFB-Vout計算シート](./v4.0/calc.gnumeric)
から、充電中は 1.7V くらいになることがあるというとで、そこから充電用に 0.5V 上げるとして
2.2V が出せる VFB を考えると、
...
式を見やすくするため、また、R1, R2 の絶対値は重要ではないため、式7
```
\alpha=\frac{R2}{R1+R2}
```
