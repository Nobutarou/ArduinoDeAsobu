...
Murata や Panasonic のリチウムイオン電池は信頼できるところからは入手ができないので、エネル
ープを 3.3V に昇圧してみる。

DCDC は秋月の Torex XCL103 を使ったやつにする。エネループ 1本で 3.3V に昇圧できる。XCL102
を使ったものもあるが、こっちは低ノイズ用途で、XCL103 が高効率用途ということで、今回はこっ
ち。

https://akizukidenshi.com/catalog/g/g116116/

XCL103 は 100mA くらいが限界のようである。なので 3個が 4個ほど並列にして、電源用のショット
キーを挟んでまとめ上げることにする。設計は 4個差せるようにしておいて、実際は 3個から始めて
みよう。

VF=3.1V だから、抵抗無しでも行けそうだが、そもそも v2.0, v2.1 の LED 基板部分を流用するか
ら、若干暗くなりそう。必要なら、抵抗を抜くか 1Ωとかチキンと入れておこう。

なんかこれまで回路図を書いてなかったみたいだから、回路図を書こう。

[回路図](./v3.0/LEDlight_3.0/LEDlight_3.0_Schematics.pdf)

[設計図](./v3.0/LEDlight_3.0.pdf)
...
| ---  | ---                                | ---  |
| B1   | 基板 24x8P                         | 1    |
| D1-4 | ショットキーダイオード電源用       | 3(4) |
| J1   | 単三ケース                         | 1    |
| S2   | MS-500C                            | 1    |
| U1-4 | 秋月 XCL103 キット                 | 3(4) |
| X1   | XH ヘッダ 2P                       | 1    |
| Z1-3 | 3.3mm 高さソケット 1P              | 3    |
| Z4   | 3.3mm 高さソケット 2P              | 1    |

![完成品](./v3.0/LEDLight_v3.0.jpg)

結局 v2.1 のライト部分をそのまま流用できた。

ショットキーを通った後に、ライト部への投入電圧は、ちょうど 3.0V だった。
v2.1 のライトは vf=3.1V で 10Ωを入れてあるから、そのうち 1Ω程度に入れ変えておこう。
