...
| --- | --- | --- | --- | --- |
| Z | Z | L | H | TXD に Z は無いから無効のモード | 
| Z | L | L | L | TXD に Z は無いから無効のモード |
| Z | H | H | L | TXD に Z は無いから無効のモード |
| L | Z | L | L | |
| L | L | L | L | TXD が動作しているときは TOOL0 は Z なので無効のモード |
| L | H | L | L | TXD が動作しているときは TOOL0 は Z なので無効のモード |
| H | Z | H | H | |
| H | L | L | L | TXD の待機状態で H |
| H | H | H | H | TXD の待機状態で H |

有効なモードで全て一致。TXD は FT232RL と CP2102 で試したが、何も送信していない待機状態は
H のようだ。なので TXD が待機中に TOOL0 からの H, L が RXD に入り、TOOL0 が Z 中に TXD の
H, L がしっかり TOOL0 に届くので、これで良いはず。

多分 UPDI と同じような仕組みだし、それで良いのだろう。
...
Lチカ、UART 送信、I2C による MCP23017 操作による LCD 操作にも成功
[プロジェクトフォルダ](./e2studio/)
ごと置いておいた
<!-- }}} -->
...
[回路図](./v1.1/R7F101G6E2DSP_1.1/R7F101G6E2DSP_1.1.pdf)
[設計図](./v1.1/R7F101G6E2DSP_1.1.pdf)

# v1.2

主な変更点は電源ラインの増強。部品は同じ

[設計図](./v1.2/R7F101G6E2DSP_1.2.pdf)

# v1.3

あまり配線を飛ばしまくると、作りにくいと感じたので、電源ラインは表、GND は裏で太くするよう
にした。部品表、回路図は変更なし。

[設計図](./v1.3/R7F101G6E2DSP_1.3.pdf)
