...
なんどもやらかしてマザボの USB の電源ラインがちょっと変になってる。電子工作をやってるとど
うしても事故は 0 にはできない。なので、USB の一番の上流となるここで、電源を実験電源から取
り、失敗しても PC は壊れないように分岐させる。

[回路図](./v3.0/USBSwitcher_3.0/USBSwitcher_3.0_Schematics.pdf)

[設計図](./v3.0/TC7USB40FT_3.0.pdf)

部品表:
| 記号 | 品番、品目              | 個数 |
| ---  | ----------              | ---- |
|B1    | ユニバーサル基板 15x14P | 1 |
|C1,2  | セラコン 1.5uF          | 2 |
|C3    | セラコン 0.22uF         | 1 |
|J1,5  | XH ポスト 横向き 2P     | 2 |
|J2,3  | USB-B コネクタ          | 2 |
|J4    | USB-A コネクタ          | 1 |
|R1    | 100kΩ                  | 1 |
|S1,2  | ピンソケット 5P         | 2 |
|U1    | TC7USB40FT              | 1 |
|U2    | S-812C33AY-B-G 3.3V LDO | 1 |

完成写真を取る前に組み込んでしまった。ま、無事、動作中。
...
- 主な 5V と GND 線は基板のパターンを使わずに導線を直結する
- ラインが交差してしまうときは、90度を守る

部品表:

|記号    | 品番、品目                             |個数|
|--------|----------------------------------------|----|
|B1      | ユニバーサル基板 15x16P                |1   |
|C1      | 1uF 程度、種別問わず                   |1   |
|C2      | 10uF以下のセラコン                     |1   |
|C3      | 0.1uF 程度のセラコン                   | 1  |
|H1      | ピンヘッダ 3P                          |1   |
...
[設計図](./TC7USB40FT/v2.1/TC7USB40FT_2.1.pdf)

S 端子にテスターを当てると、プルアップしてるのに 1V くらいになることが判明。もちろん、テス
ターを当てたせいかもしれない。しかし IC 単体でも 3.7kΩくらいが S-GND 間の抵抗として見える
ことや、テスターで確実にテストできることを考えると、次回作は、プルダウンすべきであろう。

![完成品](./TC7USB40FT/v2.1/TC7USB40FT_v2.1.jpg)

5V ラインは AWG24 のより線で、GND は主ラインをすずめっき線で増強。
...
→ [ここの検証](../misc/LDO/S-812C33AY-B-G/README.md)
によると、S-812C33AY-B-G にはコンデンサ要らなそう。ただ、安心感のために CIN=CL=1.5uF を付
けることにする。ほぼ USB コネクタで場所を占めているので、コンデンサを省いたところで小さく
はできないから。
