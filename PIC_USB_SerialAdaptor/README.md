...
しばらく ASOBoard と一体式にしていたので単体で作ってなかったけど、1段目で最低限機能として
は完成させて、2段目を目的に応じて好きにアタッチメントを作る形にしてみようと思う。

実際のところ v1.2 までと大きな変更は無い。1段目に 3.3V LDO 向けのセラコンを実装するくらい。

最初の動機は AVR64 マイコン。どうやら USB シリアルアダプタを改造して UPDI プログラマにする
らしい。となると、アタッチメントだけ付け変えて USB シリアルとしても UPDI プログラマとして
も使える状況が良いという気がした。

またケーブルが長いことを嫌って ASOBoard と一体化したが、
[LCD アダプタ](../LM162AT1_SharpLCD/Adaptor.md) を作ったときに、ケーブルを使わずピンヘッダ
とピンソケットで合体する形として、これがすっきりしていたので、この方式なら良いと感じた。

また一体式だと USB コネクタの向きが決まってしまい、ちょっと窮屈な取り回しになるとかあった
けど、アタッチメント式なら、自分の設計力次第だけど、自由に USB コネクタの向きを決められる。

[設計図](./librecad/pic16f1455_uart_v2.0.pdf)

[回路図](./kicad/pic16f1455_serial_adaptor_2.0/pic16f1455_serial_adaptor_2.0.pdf)
