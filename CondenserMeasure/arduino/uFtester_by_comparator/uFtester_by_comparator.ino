...
void loop() {
  ...
  // 本当は割り込んだら break とかしたいけど、分からない
    ...
    Serial.println(c, 16);
    short td=-_R*c/log(1.0/500.0)/1.0e3 *2; //ms, *2 は余裕を見て
    td = (td>1000)*td + (td<=1000)*1000; //最低でも 1s。出力が速すぎても困るし 
    discharge(td);
  }
  Serial.end();
  Serial.begin(9600);
