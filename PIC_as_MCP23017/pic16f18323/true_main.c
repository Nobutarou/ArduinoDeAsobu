...
// CONFIG2
#pragma config MCLRE = ON       // RA3 を使わない
...
// I/Oポートのマッピング
// 14pinのマッピング A port=|x|x|x|x||x|A2|A1|A0|, B port=|x|x|C5|C4|C3|C2|C1|C0|
#define REG_MAP_IN			(PORTA&0b111)
#define REG_MAP_IN_B		(PORTC&0b111111)
#define REG_MAP_DIR(x)		{TRISA=0b111000|(x)&0b000111;} // RA4,5 は常に tri state
#define REG_MAP_DIR_B(x)	{TRISC=(x)&0b111111;}	// 
#define REG_MAP_PU(x)		{WPUA=0b111000|(x)&0b000111;} // RA3~5 は常にプルアップ
#define REG_MAP_PU_B(x)		{WPUC=(x)&0b111111;}	
#define REG_MAP_LAT(x)		{LATA=(x)&0b000111;}
#define REG_MAP_LAT_B(x)	{LATC=(x)&0b111111;}
...
void InitPic(void)
{
  ...
  SSP1CLKPPS = 0b00101; // SCL は RA5
  RA5PPS     = 0b11000; // RA5 は SCL
  SSP1DATPPS = 0b00100; // SDA は RA4
  RA4PPS     = 0b11001; // RA4 は SDA
}
